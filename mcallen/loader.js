(async()=>{
  ...
  try{
    const html=(await Promise.all(parts.map(async p=>{const r=await fetch('./'+p,{cache:'no-cache'});if(!r.ok)throw new Error(p);return r.text()}))).join('');
    document.getElementById('app').outerHTML=html;
    for(const src of ['data.js','core.js','features.js','search.js','share.js']){
      await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='./'+src+'?v=4.0.0';s.onload=resolve;s.onerror=reject;document.body.appendChild(s)});
    }
  }catch(error){
